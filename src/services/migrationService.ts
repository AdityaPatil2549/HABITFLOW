...
// and batch-upserts everything to Firestore.
// ============================================================
...
export async function migrateLocalDataToCloud(userId: string): Promise<MigrationProgress> {
  ...
  if (!isFirebaseConfigured()) {
    console.warn('[MigrationService] Firebase not configured — skipping migration.');
    return progress;
  }
  ...
  const dexieTableNames = Object.keys(DEXIE_TO_FIRESTORE);
  ...
  for (const dexieTableName of dexieTableNames) {
    ...
    const firestoreTable = DEXIE_TO_FIRESTORE[dexieTableName];

    const rows = records.map((record: Record<string, any>) =>
      toFirestoreRow(firestoreTable, {
        ...record,
        user_id: userId,
        updated_at: now,
      })
    ...
    const BATCH_SIZE = 500; // Firestore limit is 500
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      try {
        for (const row of chunk) {
          const docRef = doc(firestore, firestoreTable, String(row.id));
          batch.set(docRef, row, { merge: true });
        }
        await batch.commit();
        progress.synced += chunk.length;
      } catch (err) {
        console.error(`[MigrationService] Network error migrating ${firestoreTable}:`, err);
      }
    }
  }
  ...
  console.info(`[MigrationService] Migration complete: ${progress.synced}/${progress.total} records synced.`);
  return progress;
}
