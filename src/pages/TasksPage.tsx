      ...
      <div className="flex gap-3 pt-1">
        ...
        <button
          ...
          style={{
            ...
            boxShadow: '0 8px 20px rgba(var(--brand-500-rgb),0.3)',
          }}
        >
...
function TaskItem({ task, depth = 0 }: { task: Task; depth?: number }) {
  ...
  if (editing)
    return (
      <motion.div
        layout
        className={cn('rounded-2xl p-4 glass-card')}
        style={{
...
export function TasksPage() {
  ...
  useEffect(() => {
    document.title = 'Tasks — HabitFlow';
  }, []);
        ...
        {VIEWS.map(v => (
          ...
          >
            ...
            {v === 'Today' && totalToday > 0 && (
              <span className="ml-1.5 text-[10px] bg-brand-500/20 text-brand-active px-1.5 py-0.5 rounded-full">
                {totalToday}
              </span>
            )}
          </button>
