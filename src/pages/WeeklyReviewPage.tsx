...
export function WeeklyReviewPage() {
  ...
  const bestHabit = habits.length
    ? [...habits].sort((a, b) => b.completionRate30Days - a.completionRate30Days)[0]
    : null;
  const strugglingHabit = habits.length
    ? ([...habits]
        .filter(h => h.completionRate30Days < 0.5)
        .sort((a, b) => a.completionRate30Days - b.completionRate30Days)[0] ?? null)
    : null;
      ...
      {loading ? (
        ...
        <>
          <div className="text-center relative mb-10 z-10">
            ...
            <p className="whitespace-nowrap text-slate-400 text-sm sm:text-base font-medium leading-relaxed italic opacity-80 mt-2">
          ...
          <div className="space-y-6 z-10 relative">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-black text-amber-400">
                        {Math.round(bestHabit.completionRate30Days * 100)}%
                      </p>
                    </div>
                  </div>
                  <div className="w-full bg-white/5 rounded-full h-1.5 mt-2 overflow-hidden">
                    <div
                      className="bg-gradient-to-r from-amber-500 to-yellow-300 h-1.5 rounded-full"
                      style={{ width: `${Math.round(bestHabit.completionRate30Days * 100)}%` }}
                    />
                  </div>
                </div>
              )}
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-black text-emerald-400">
                        {Math.round(strugglingHabit.completionRate30Days * 100)}%
                      </p>
                    </div>
                  </div>
                  <div className="w-full bg-white/5 rounded-full h-1.5 mt-2 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.round(strugglingHabit.completionRate30Days * 100)}%`,
                      }}
                    />
                  </div>
                </div>
            ...
            {xpData && stats && (
              <motion.div
                ...
                className="bg-white/[0.02] backdrop-blur-md border border-white/10 rounded-3xl p-6 sm:p-8 space-y-6"
              >
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex-1">
                    <h2 className="text-xl font-bold text-white mb-1">
                      Level {numericLevel} — {xpData.level}
                    </h2>
                    <p className="text-sm text-slate-400">
                      You earned {xpData.weeklyScore} XP this week!
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="w-full bg-white/5 rounded-full h-2.5 overflow-hidden">
                    <div
                      className="bg-gradient-to-r from-indigo-500 via-purple-500 to-fuchsia-500 h-2.5 rounded-full transition-all duration-1000"
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-white/5">
                  ...
                  >
                    {isGenerating ? (
                      ...
                      <>
                        <Share2 size={18} /> Share Milestone
                      </>
                    )}
                  </button>
