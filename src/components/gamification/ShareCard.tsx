    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center -mt-10">
          <p className="text-3xl font-bold uppercase tracking-[0.2em] text-brand-400 mb-6">
            {subtitle}
          </p>
          ...
          <div className="grid grid-cols-2 gap-8 w-full max-w-4xl">
            {stats.map((stat, i) => (
              <div
                key={i}
                className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[3rem] p-10 flex flex-col items-center gap-4 shadow-2xl relative overflow-hidden"
              >
                <div className="w-24 h-24 rounded-full bg-white/5 flex items-center justify-center shadow-inner mb-2">
                  {ICON_MAP[stat.icon]}
                </div>
                <p className="text-[5.5rem] leading-none font-black text-white tracking-tight">
                  {stat.value}
                </p>
                <p className="text-2xl font-bold text-slate-400 uppercase tracking-widest mt-2">
                  {stat.label}
                </p>
              </div>
            ))}
