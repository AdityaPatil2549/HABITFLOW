...
interface NeonCheckboxProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> {
  ...
  neonColor?: string; // e.g. '#00ffaa'
  checkboxSize?: string; // e.g. '30px'
...
const NeonCheckbox: React.FC<NeonCheckboxProps> = ({ 
  ...
  const neonCheckboxStyles = {
    '--primary': neonColor,
    '--primary-dark': `${neonColor}80`, // pseudo darker version
    '--primary-light': `${neonColor}dd`,
    '--size': checkboxSize,
  } as React.CSSProperties;

  return (
    <label 
      className={`relative inline-block w-[var(--size)] h-[var(--size)] cursor-pointer flex-shrink-0 ${className}`} 
      style={neonCheckboxStyles}
      onClick={(e) => e.stopPropagation()} // prevent parent clicks if it's in a list item
    >
      <input 
        type="checkbox" 
        className="hidden" 
      ...
      <div className="relative w-full h-full neon-checkbox__frame">
        <div className={`absolute inset-0 bg-black/80 rounded-xl border-2 transition-all duration-400 neon-checkbox__box ${
          isChecked 
            ? 'border-[var(--primary)] bg-[rgba(var(--primary),0.1)]' 
            : 'border-[var(--primary-dark)] bg-slate-900/50'
        }`}>
          <div className="absolute inset-[2px] flex items-center justify-center neon-checkbox__check-container z-10">
            <svg 
              viewBox="0 0 24 24" 
          ...
          <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${isChecked ? 'opacity-0' : 'opacity-100'}`}>
            {children}
          </div>
          ...
          <div className="absolute inset-0 rounded-xl overflow-hidden neon-checkbox__borders">
            {[...Array(4)].map((_, i) => (
              <span 
                key={i} 
      ...
      {label && (
        <span className="ml-10 text-white flex items-center h-full whitespace-nowrap">{label}</span>
      )}
