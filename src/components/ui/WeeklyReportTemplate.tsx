    ...
    >
      <div className="flex justify-between items-end mb-8 border-b border-white/10 pb-6">
        ...
        <div className="text-right">
          ...
          <div className="text-sm font-bold text-slate-500 uppercase tracking-widest">Active Habits</div>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-6">
        {activeHabits.slice(0, 6).map(habit => (
          <div key={habit.id} className="bg-white/5 rounded-2xl p-5 border border-white/5 flex justify-between items-center">
            <div>
      ...
      <div className="mt-8 text-center border-t border-white/10 pt-6">
        <p className="text-slate-500 font-bold tracking-widest uppercase text-sm">Powered by HabitFlow</p>
      </div>
    </div>
  );
});
